...
public final class ProviderGroupListTest extends ActionsListTest {
  private static final String group = "group";
  @Rule public TemporaryFolder temporaryFolder = new TemporaryFolder();
  private ReactiveCache reactiveCache;
  private ProviderGroupList<Mock> cacheProvider;

  @Before public void setUp() {
    reactiveCache = new ReactiveCache.Builder()
        .using(temporaryFolder.getRoot(), Jolyglot$.newInstance());
    cacheProvider = reactiveCache.<Mock>providerGroupList()
        .withKey("mock");
  }
  ...
  @Test public void Verify_Inherited_Api() {
    ...
    TestObserver<List<Mock>> observer = cacheProvider.read(group)
        .test();
    ...
    //read throws
    observer = cacheProvider.read(group).test();
    observer.awaitTerminalEvent();
    ...
    observer = Single.just(Arrays.asList(new Mock(message)))
        .compose(cacheProvider.readWithLoader(group)).test();
    observer.awaitTerminalEvent();
    observer
        .assertValueAt(0, mocks -> mocks.get(0).getMessage().equals("1"))
        .assertNoErrors()
  ...
  @Override protected ActionsList<Mock> actions() {
    return cacheProvider.entries(group);
  }

  @Override protected Single<List<Mock>> cache() {
    return cacheProvider.read(group);
  }
}
